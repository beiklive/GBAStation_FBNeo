...
public:
    ...
    // loaded once at init.  The menu pauses the core, so PushSample() never
    // runs while it is open: in SDL_QueueAudio mode the clip is pushed straight
    // into the device queue and plays immediately.
    //--------------------------------------------------------------------------
    enum class UiSound
    {
        Focus,
