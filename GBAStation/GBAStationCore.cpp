...
/// @brief Simplified libretro frontend for fbneo with GBAStation overlay
...
#define GBAStation_debug_log(...) LOG_CORE(__VA_ARGS__)

void GBAStationCore::LoadSaveData()
{
    ...
    std::string gameSaveDir = std::string(GBAStationConfig::SAVES_PATH) + filename + "/";
    std::string savePathSav = gameSaveDir + filename + ".sav";
    std::string savePathSrm = gameSaveDir + filename + ".srm";

    std::ifstream fileSav(savePathSav, std::ios::binary);
    if (fileSav)
    {
        fileSav.read((char *)data, size);
        GBAStation_debug_log("Loaded SRAM from %s", savePathSav.c_str());
        return;
    }
    ...
    if (fileSrm)
    {
        ...
        GBAStation_debug_log("Loaded SRAM from %s (legacy fallback)", savePathSrm.c_str());
    }
}

void GBAStationCore::SaveSaveData()
{
    size_t size = retro_get_memory_size(RETRO_MEMORY_SAVE_RAM);
    if (!size)
        return;
    ...
    std::string gameSaveDir = std::string(GBAStationConfig::SAVES_PATH) + filename + "/";
    if (stat(gameSaveDir.c_str(), &st) == -1)
        mkdir(gameSaveDir.c_str(), 0777);

    std::string savePath = gameSaveDir + filename + ".sav";

    std::ofstream file(savePath, std::ios::binary);
    if (file)
    {
        file.write((const char *)data, size);
        GBAStation_debug_log("Saved SRAM to %s", savePath.c_str());
    }
}
...
// Forward declarations for fbneo core functions (C linkage)
extern "C"
{
        ...
        // Queue result callback for main thread
        {
            ...
            self->m_raPendingCallbacks.push_back(
                [job, http_code, readBuffer, errorMsg, requestUrl]() {
                    GBAStation_debug_log("RA: HTTP Request -> %s", requestUrl.c_str());
                    if (!errorMsg.empty()) {
                        GBAStation_debug_log("RA HTTP Error: %s", errorMsg.c_str());
                    }
                    GBAStation_debug_log("RA: HTTP Response %ld (size: %zu)", http_code, readBuffer.size());
...
void GBAStationCore::StartRAWorker() {
    ...
    if (R_SUCCEEDED(rc)) {
        ...
        if (R_SUCCEEDED(rc)) {
            ...
            GBAStation_debug_log("RA: Worker thread started (core 0, 256KB stack)");
        } else {
            GBAStation_debug_log("RA: threadStart failed: 0x%x", rc);
            threadClose(&m_raThread);
            m_raWorkerRunning = false;
        }
    } else {
        GBAStation_debug_log("RA: threadCreate failed: 0x%x", rc);
        m_raWorkerRunning = false;
    }
#endif
}
...
void GBAStationCore::StopRAWorker() {
    ...
    GBAStation_debug_log("RA: Worker thread stopped");
#endif
}
...
static void RC_CCONV RAServerCall(const rc_api_request_t* request, rc_client_server_callback_t callback, void* callback_data, rc_client_t* client)
{
    ...
    if (s_instance->m_raWorkerRunning) {
        ...
        GBAStation_debug_log("RA: HTTP Request (sync) -> %s", request->url);
        CURL *curl = curl_easy_init();
        std::string readBuffer;
        long http_code = 0;
        if (curl) {
            ...
            else { GBAStation_debug_log("RA HTTP Error: %s", curl_easy_strerror(res)); http_code = 500; }
            curl_easy_cleanup(curl);
        }
        GBAStation_debug_log("RA: HTTP Response %ld (size: %zu)", http_code, readBuffer.size());
        rc_api_server_response_t response;
    ...
    GBAStation_debug_log("RA: HTTP Request -> %s", request->url);
    CURL *curl = curl_easy_init();
    std::string readBuffer;
    long http_code = 0;
    if (curl) {
...
GBAStationCore::~GBAStationCore()
{
    GBAStation_debug_log("~GBAStationCore: destroying (gameLoaded=%d, initialized=%d, hwRender=%d)",
    ...
    if (m_initialized)
    {
        ...
        GBAStation_debug_log("Calling retro_deinit...");
        retro_deinit();
        GBAStation_debug_log("retro_deinit done");
        m_initialized = false;
    }
    ...
    GBAStation_debug_log("~GBAStationCore: done");
}
...
bool GBAStationCore::Init()
{
    ...
    GBAStation_debug_log("=== GBAStationCore::Init() ===");
    ...
    if (!s_vibrationInitialized)
    {
        ...
        GBAStation_debug_log("Vibration devices initialized for P1-P4");
    }
#endif
    ...
    GBAStation_debug_log("System dir: %s", m_systemDir.c_str());

    // Load configuration to ensure variables are ready for init
    LoadConfig();
    LoadRAConfig();
    GBAStation_debug_log("Config loaded, %lu options", m_configOptions.size());
...
#else
        m_trophySound = Mix_LoadWAV("GBAStation/assets/trophy.mp3");
#endif
        if (m_trophySound) GBAStation_debug_log("RA: Loaded trophy.mp3 successfully.");
        else GBAStation_debug_log("RA: Failed to load trophy.mp3 -> %s", Mix_GetError());
    }

    // Environment callback must be set before retro_init
    GBAStation_debug_log("Calling retro_set_environment...");
    retro_set_environment(EnvironmentCallback);
    GBAStation_debug_log("retro_set_environment done");

    // Initialize core
    GBAStation_debug_log("Calling retro_init...");
    retro_init();
    GBAStation_debug_log("retro_init done");

    // Initialize RetroAchievements
    m_rcClient = rc_client_create(RAReadMemory, RAServerCall);
    if (m_rcClient) {
        GBAStation_debug_log("RA: Client created");
            ...
            switch (event->type) {
                case RC_CLIENT_EVENT_ACHIEVEMENT_TRIGGERED:
                    if (event->achievement) {
                        ...
                        GBAStation_debug_log("RA: Achievement triggered: %s (badge: %s)", title.c_str(), badge.c_str());
                    }
                    break;
                ...
                case RC_CLIENT_EVENT_SERVER_ERROR:
                    if (event->server_error) {
                        GBAStation_debug_log("RA: Server error: %s", event->server_error->error_message);
                    }
                    break;
                default:
                    break;
    ...
    GBAStation_debug_log("Initialized: %s %s",
...
void GBAStationCore::SetHWRenderContext(SDL_Window *window, EGLContext mainCtx, EGLContext hwCtx)
{
    m_window = window;
...
bool GBAStationCore::InitEGLDualContext()
{
    ...
    {
        GBAStation_debug_log("ERROR: Failed to get current EGL context");
        return false;
    }
    ...
    {
        GBAStation_debug_log("ERROR: FBO incomplete: 0x%x", status);
    ...
    GBAStation_debug_log("Created HW render texture: %u (%dx%d) FBO: %u",
...
void GBAStationCore::DestroyHWRenderContext()
{
    if (!m_hwRender || !s_hwRenderCallback.context_destroy)
        return;

    GBAStation_debug_log("Calling context_destroy...");
    glFinish();
    s_hwRenderCallback.context_destroy();
    GBAStation_debug_log("context_destroy done");
...
{
    GBAStation_debug_log("=== GBAStationCore::LoadGame ===");
    GBAStation_debug_log("  path: %s", path.c_str());

    m_gamePath = path;

    if (!m_initialized)
    {
        GBAStation_debug_log("Not initialized, calling Init()");
        if (!Init())
        {
            GBAStation_debug_log("ERROR: Init() failed");
            return false;
        }
    }

    GBAStation_debug_log("Opening ROM file...");

    // need_fullpath = false: load ROM into memory
    FILE *fp = fopen(path.c_str(), "rb");
    if (!fp)
    {
        GBAStation_debug_log("ERROR: Failed to open file: %s", path.c_str());
        return false;
    }
    ...
    {
        fclose(fp);
        GBAStation_debug_log("ERROR: File is empty: %s", path.c_str());
        return false;
    }

    GBAStation_debug_log("ROM size: %zu bytes (%.1f MB)", fileSize, fileSize / (1024.0 * 1024.0));
    ...
    {
        GBAStation_debug_log("ERROR: Short read: %zu of %zu bytes", bytesRead, fileSize);
        return false;
    }
    ...
    GBAStation_debug_log("Calling retro_load_game...");
    GBAStation_debug_log("  gameInfo.path = %s", gameInfo.path);
    GBAStation_debug_log("  gameInfo.size = %zu", gameInfo.size);

    if (!retro_load_game(&gameInfo))
    {
        GBAStation_debug_log("ERROR: retro_load_game failed");
        return false;
    }
    GBAStation_debug_log("retro_load_game succeeded");

    // Get AV info
    GBAStation_debug_log("Getting AV info...");
    ...
    GBAStation_debug_log("AV info: %dx%d @ %.2f fps, %.0f Hz, aspect %.3f",
             m_frameWidth, m_frameHeight, m_fps, m_sampleRate, m_aspectRatio);

    // Set up FBO and trigger deferred context_reset
    if (m_hwRender)
    {
        GBAStation_debug_log("Initializing HW render context...");
        if (InitEGLDualContext())
        {
            if (s_hwRenderCallback.context_reset)
            {
                GBAStation_debug_log("Calling context_reset...");
                s_hwRenderCallback.context_reset();
                GBAStation_debug_log("context_reset done");
            }
            else
            {
                GBAStation_debug_log("WARNING: No context_reset callback!");
            }
        }
        else
        {
            GBAStation_debug_log("ERROR: InitEGLDualContext failed");
        }
    }
    else
    {
        GBAStation_debug_log("Software rendering mode (no HW render requested)");
    }

    // Set controller - N64 uses standard joypad
    GBAStation_debug_log("Setting controller port devices...");
    ...
    m_gameLoaded = true;
    m_paused = false;
    GBAStation_debug_log("LoadGame: Complete!");
    ...
    if (m_rcClient && m_raEnabled && !m_raUsername.empty()) {
        ...
        if (!m_raToken.empty()) {
            ...
            GBAStation_debug_log("RA: Beginning login with token...");
            rc_client_begin_login_with_token(m_rcClient, m_raUsername.c_str(), m_raToken.c_str(),
                [](int res, const char* err, rc_client_t* c, void* ud) {
                    ...
                    if (res == RC_OK) {
                        GBAStation_debug_log("RA: Token login successful!");
                        RAIdentifyGame(c, core);
                    } else {
                        GBAStation_debug_log("RA: Token login failed: %s. Retrying with password...", err ? err : "Unknown");
                        RALoginWithPassword(c, core);
                    }
                }, this);
        } else if (!m_raPassword.empty()) {
            // No token, try password directly
            GBAStation_debug_log("RA: No token, logging in with password...");
            RALoginWithPassword(m_rcClient, this);
        } else {
            GBAStation_debug_log("RA: No token or password configured. Skipping RA.");
        }
    }
...
void GBAStationCore::UnloadGame()
{
    ...
    GBAStation_debug_log("Calling retro_unload_game...");
    retro_unload_game();
    GBAStation_debug_log("retro_unload_game done");
    ...
    // Delete FBO objects
    GBAStation_debug_log("Deleting GBAStationCore GL objects (tex=%u fbo=%u rbo=%u)",
    ...
    GBAStation_debug_log("UnloadGame GL cleanup complete");
}
...
void GBAStationCore::ResizeFBO(int width, int height)
{
    ...
    GBAStation_debug_log("ResizeFBO: %dx%d", width, height);
    ...
    if (status != GL_FRAMEBUFFER_COMPLETE)
    {
        GBAStation_debug_log("ERROR: ResizeFBO incomplete: 0x%x", status);
    }
    else
    {
...
void GBAStationCore::SaveState(const std::string &path)
{
    ...
    if (size == 0)
    {
        GBAStation_debug_log("SaveState: size 0");
        BindHWContext(false);
        return;
    }
    ...
    if (success)
    {
        ...
        if (fp)
        {
            ...
            fclose(fp);
            GBAStation_debug_log("Saved state to %s", path.c_str());
        }
        else
        {
            GBAStation_debug_log("ERROR: Failed to open file for save state: %s", path.c_str());
        }
    }
    else
    {
        GBAStation_debug_log("ERROR: retro_serialize failed");
    }
}
...
void GBAStationCore::LoadState(const std::string &path)
{
    ...
    if (!fp)
    {
        GBAStation_debug_log("LoadState: File not found: %s", path.c_str());
        return;
    }
    ...
    {
        GBAStation_debug_log("Resetting SDL audio device...");
    ...
    if (success)
    {
        GBAStation_debug_log("Loaded state from %s", path.c_str());
    }
    else
    {
        GBAStation_debug_log("ERROR: retro_unserialize failed");
    }
}
...
bool GBAStationCore::SetRumbleStateCallback(unsigned port, enum retro_rumble_effect effect, uint16_t strength)
{
#ifdef __SWITCH__
...
//==============================================================================
bool GBAStationCore::ClearThreadWaitsCallback(unsigned cmd, void *data)
{
...
bool GBAStationCore::HandleEnvironment(unsigned cmd, void *data)
{
    ...
    case RETRO_ENVIRONMENT_GET_RUMBLE_INTERFACE:
    {
        ...
        if (cb) {
            ...
            GBAStation_debug_log("ENV: Provided Rumble Interface");
            return true;
        }
        return false;
    }
    ...
    case RETRO_ENVIRONMENT_GET_SYSTEM_DIRECTORY:
    {
        *(const char **)data = m_systemDir.c_str();
        GBAStation_debug_log("ENV: GET_SYSTEM_DIRECTORY -> %s", m_systemDir.c_str());
        return true;
    }
    ...
    case RETRO_ENVIRONMENT_GET_SAVE_DIRECTORY:
    {
        *(const char **)data = m_saveDir.c_str();
        GBAStation_debug_log("ENV: GET_SAVE_DIRECTORY -> %s", m_saveDir.c_str());
        return true;
    }
    ...
    case RETRO_ENVIRONMENT_SET_PIXEL_FORMAT:
    {
        ...
        if (fmt) {
            m_pixelFormat = *fmt;
            GBAStation_debug_log("ENV: SET_PIXEL_FORMAT to %d", m_pixelFormat);
            return true;
        }
        return false;
    }
    ...
    case RETRO_ENVIRONMENT_SET_HW_RENDER:
    {
        ...
        GBAStation_debug_log("ENV: SET_HW_RENDER accepted - context_type=%d, version=%d.%d",
    ...
    case RETRO_ENVIRONMENT_SET_MEMORY_MAPS:
    {
        ...
        if (mem_map) {
            ...
            GBAStation_debug_log("ENV: SET_MEMORY_MAPS (%u descriptors processed)", mem_map->num_descriptors);
        }
        return true;
    }
    ...
    case RETRO_ENVIRONMENT_SET_SYSTEM_AV_INFO:
    {
        ...
        GBAStation_debug_log("ENV: SET_SYSTEM_AV_INFO: base %dx%d, FBO %dx%d @ %.2f fps",
    ...
    case RETRO_ENVIRONMENT_GET_CLEAR_ALL_THREAD_WAITS_CB:
    {
        if (data) {
            *(retro_environment_t *)data = ClearThreadWaitsCallback;
            GBAStation_debug_log("ENV: GET_CLEAR_ALL_THREAD_WAITS_CB provided");
            return true;
        }
        return false;
    }
    ...
    default:
        ...
        if (base_cmd != 47 && base_cmd < 100) {
            GBAStation_debug_log("ENV: Unhandled cmd %u (0x%x) -> false", cmd, cmd);
        }
        break;
    }
...
void GBAStationCore::LoadConfig()
{
...
#ifdef __SWITCH__
    configPath = "sdmc:/GBAStation/config/cores/fbneo.jsonc";
#else
    configPath = "GBAStation/config/cores/fbneo.jsonc";
#endif

    // config.cfg belongs to the launcher.  Import its FBNeo namespace before
    // loading the persistent JSONC file so a chained launch never ignores the
    // settings page values.
    const auto importGBAStationOverrides = [this]() {
        ...
        for (const char *path : paths) {
            ...
            while (std::getline(cfg, line)) {
                ...
                constexpr const char *prefix = "core.fbneo.";
                ...
                m_configOptions[key.substr(std::char_traits<char>::length(prefix))] = value;
                ++count;
            }
            GBAStation_debug_log("Applied %u FBNeo config.cfg core overrides from %s", count, path);
            return;
        }
    };
    ...
    if (!f.good())
    {
        GBAStation_debug_log("No config found at %s. Using defaults.", configPath);
        importGBAStationOverrides();
    ...
    if (j.is_discarded())
    {
        GBAStation_debug_log("ERROR: Failed to parse config at %s", configPath);
        m_configLoaded = true;
        return;
    }
    ...
    m_configLoaded = true;
    GBAStation_debug_log("Loaded %lu options from %s", m_configOptions.size(), configPath);
}

void GBAStationCore::LoadRAConfig()
{
    std::string accountsPath = "sdmc:/GBAStation/config/accounts.jsonc";
    std::ifstream file(accountsPath);
    if (!file.is_open()) {
        GBAStation_debug_log("WARN: accounts.jsonc not found at %s", accountsPath.c_str());
        return;
    }

    GBAStation_debug_log("RA: Found accounts.jsonc at %s", accountsPath.c_str());
    ...
    if (!j.is_discarded() && j.is_object()) {
        ...
        GBAStation_debug_log("RA: Config loaded (Enabled: %d, User: %s, HasToken: %d, HasPassword: %d)",
            m_raEnabled, m_raUsername.c_str(), !m_raToken.empty(), !m_raPassword.empty());
    } else {
        GBAStation_debug_log("WARN: Failed to parse accounts.jsonc for RA settings.");
    }
}
...
void GBAStationCore::SaveRAToken(const std::string& token)
{
    ...
    if (outFile.is_open()) {
        outFile << j.dump(4);
        outFile.close();
        GBAStation_debug_log("RA: Token saved to accounts.jsonc");
    } else {
        GBAStation_debug_log("RA: WARNING - Failed to save token to %s", accountsPath.c_str());
    }
}

void GBAStationCore::RAIdentifyGame(rc_client_t* c, GBAStationCore* core)
{
    GBAStation_debug_log("RA: Identifying game...");
    ...
    rc_client_begin_identify_and_load_game(c, 27, core->m_gamePath.c_str(), nullptr, 0,
        [](int result, const char* error_message, rc_client_t* client, void* userdata) {
            GBAStationCore* core = (GBAStationCore*)userdata;
            if (result == RC_OK) {
                GBAStation_debug_log("RA: Game loaded and identified!");
                ...
                GBAStation_debug_log("RA: Failed to identify game: %s", error_message ? error_message : "Unknown");
                core->PushRANotification("RetroAchievements", 
...
void GBAStationCore::RALoginWithPassword(rc_client_t* c, GBAStationCore* core)
{
    if (core->m_raPassword.empty()) {
        GBAStation_debug_log("RA: No password configured. Continuing without RA.");
        return;
    }
    
    GBAStation_debug_log("RA: Logging in with password...");
    rc_client_begin_login_with_password(c, core->m_raUsername.c_str(), core->m_raPassword.c_str(),
        [](int res, const char* err, rc_client_t* c, void* ud) {
            ...
            if (res == RC_OK) {
                ...
                if (user && user->token) {
                    GBAStation_debug_log("RA: Password login successful! Saving token...");
                    core->SaveRAToken(user->token);
                } else {
                    GBAStation_debug_log("RA: Password login OK but no token returned");
                }
                RAIdentifyGame(c, core);
            } else {
                GBAStation_debug_log("RA: Password login failed: %s. Continuing without RA.", err ? err : "Unknown");
                core->PushRANotification("RetroAchievements", 
...
void GBAStationCore::PushRANotification(const std::string& title, const std::string& desc,
                                   const std::string& badge)
{
    ...
    GBAStation_debug_log("RA: Notification pushed: %s - %s (badge: %s, tex: %u)",
        title.c_str(), desc.c_str(), badge.c_str(), n.textureId);
}

unsigned int GBAStationCore::GetRABadgeTexture(const std::string& badge_name)
{
    // Check cache first
    ...
    std::string path = "sdmc:/GBAStation/Arcade/assets/ra/" + badge_name + ".png";
    int w, h, ch;
...
void GBAStationCore::DownloadAndCacheBadge(const std::string& badge_name)
{
    // Check if already cached on disk
    std::string cachePath = "sdmc:/GBAStation/Arcade/assets/ra/" + badge_name + ".png";
    ...
    if (res != CURLE_OK || httpCode != 200 || response.empty()) {
        GBAStation_debug_log("RA: Failed to download badge %s (http %ld)", badge_name.c_str(), httpCode);
        return;
    }
    
    // Ensure directory exists
    mkdir("sdmc:/GBAStation/Arcade", 0777);
    mkdir("sdmc:/GBAStation/Arcade/assets", 0777);
    mkdir("sdmc:/GBAStation/Arcade/assets/ra", 0777);
    ...
    if (fp) {
        ...
        fclose(fp);
        GBAStation_debug_log("RA: Cached badge %s (%zu bytes)", badge_name.c_str(), response.size());
    }
}
...
void GBAStationCore::PreloadRABadges()
{
    ...
    GBAStation_debug_log("RA: Preloading achievement badges...");
    ...
    if (!list) {
        GBAStation_debug_log("RA: No achievement list to preload");
        return;
    }
    ...
    for (uint32_t b = 0; b < list->num_buckets; b++) {
        for (uint32_t a = 0; a < list->buckets[b].num_achievements; a++) {
            ...
            if (ach && ach->badge_name[0]) {
                ...
                std::string path = "sdmc:/GBAStation/Arcade/assets/ra/" + bn + ".png";
    ...
    if (badges.empty()) {
        GBAStation_debug_log("RA: All badges already cached");
        return;
    }
    
    GBAStation_debug_log("RA: Need to download %zu badges", badges.size());
...
void GBAStationCore::LoadRAIcon()
{
    // Try loading ra.svg - but nanosvg is only in GBAStationOverlay.
    // Instead, try loading a cached PNG version, or just skip if not available.
    // The SVG will be loaded by GBAStationOverlay which has nanosvg.
    GBAStation_debug_log("RA: LoadRAIcon called (will be loaded by overlay)");
}
...
// Shader sources are now in GBAStationShaders.cpp

unsigned int GBAStationCore::CompileShaderProgram(const char *vsSrc, const char *fsSrc)
{
    ...
    if (!success) {
        ...
        GBAStation_debug_log("Shader VS compile error: %s", log);
    }
    ...
    if (!success) {
        ...
        GBAStation_debug_log("Shader FS compile error: %s", log);
    }
    ...
    if (!success) {
        ...
        GBAStation_debug_log("Shader link error: %s", log);
    }
...
void GBAStationCore::InitShaderPipeline()
{
    ...
    GBAStation_debug_log("Shader pipeline initialized");
}

void GBAStationCore::DestroyShaderPipeline()
{
...
void GBAStationCore::SetShader(ShaderType type)
{
    ...
    int idx = (int)type;
    if (idx >= 0 && idx < 4) {
        GBAStation_debug_log("Shader set to: %s", names[idx]);
    }
}

void GBAStationCore::ApplyShader(int srcWidth, int srcHeight)
{
