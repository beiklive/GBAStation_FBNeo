...
/// @brief Overlay UI for tico-integrated fbneo (no disc support)
...
static std::string GetStatePath(TicoCore *core, int slot)
{
    ...
    std::string gameStateDir = std::string(TicoConfig::STATES_PATH) + romName + "/";
...
void TicoOverlay::LoadConfig() {
    const char *configPaths[] = {"sdmc:/GBAStation/config/display.jsonc","tico/config/display.jsonc"};
...
void TicoOverlay::LoadGeneralConfig() {
    const char *configPaths[] = {"sdmc:/GBAStation/config/general.jsonc","tico/config/general.jsonc"};
    m_hourFormat = "24h";
    FILE *fp = nullptr;
    for (const char *path : configPaths) { fp = fopen(path, "rb"); if (fp) break; }
    if (fp) {
...
static void RenderMenuItem(ImDrawList *dl, ImVec2 menuPos, ImVec2 menuSize, int i, int numItems, float itemHeight,
    ...
    if (isSelected) {
        ImU32 selCol = IM_COL32(85, 204, 255, static_cast<int>(255 * easeOut));
        dl->AddRectFilled(ImVec2(itemMin.x + 12.0f * scale, itemMin.y),
                          ImVec2(itemMax.x - 12.0f * scale, itemMax.y),
                          selCol, cornerRadius);
    }
    ImU32 textColor;
    textColor = isSelected ? IM_COL32(2,8,16,(int)(255*easeOut)) : IM_COL32(154,178,197,(int)(255*easeOut));
...
void TicoOverlay::RenderQuickMenu(ImDrawList *dl, ImVec2 displaySize) {
    ...
    dl->AddText(font, ImGui::GetFontSize() * 0.68f, ImVec2(440.0f * scale, 258.0f * scale),
                IM_COL32(154, 178, 197, static_cast<int>(220 * easeOut)),
...
void TicoOverlay::RenderSettingsMenu(ImDrawList *dl, ImVec2 displaySize) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        if (isSelected) {
            dl->AddRectFilled(ImVec2(itemMin.x + 12.0f * scale, itemMin.y),
                              ImVec2(itemMax.x - 12.0f * scale, itemMax.y),
                              IM_COL32(85, 204, 255, static_cast<int>(255 * easeOut)),
                              cornerRadius);
        }
        ...
        dl->AddText(font, fs, ImVec2(valueX, valueY), isSelected ? IM_COL32(85,204,255,(int)(255*easeOut)) : IM_COL32(238,247,255,(int)(220*easeOut)), label.c_str());
        ...
        if (isSelected) {
            dl->AddRectFilled(ImVec2(valueX - 16.0f * scale, valueY + 4.0f * scale),
                              ...
                              IM_COL32(85,204,255,(int)(255*easeOut)), 2.0f * scale);
        }
    }
}
...
void TicoOverlay::RenderHelpersBar(ImDrawList *dl, ImVec2 displaySize) {
    ...
    float easeOut = 1.0f - std::pow(1.0f - t, 3.0f);
    float scale = ImGui::GetIO().FontGlobalScale;
    float BAR_HEIGHT = 42.0f*scale, PADDING = 18.0f*scale, BUTTON_SIZE = 24.0f*scale, ITEM_SPACING = 18.0f*scale;
    ImFont *font = ImGui::GetFont(); float fontSize = ImGui::GetFontSize() * 0.78f;
    struct Helper { const char *btn; std::string desc; };
    std::vector<Helper> helpers;
    if (m_currentMenu == OverlayMenu::Settings) helpers.push_back({"方向", "调整"});
    helpers.push_back({"B", "返回"}); helpers.push_back({"A", "确定"});
    const float barX = 48.0f * scale;
    const float barY = displaySize.y - BAR_HEIGHT - 4.0f * scale;
...
#else
    std::string configPath = "tico/config/cores/fbneo.jsonc";
#endif
