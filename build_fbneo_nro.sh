...
echo "=== Building GBAStation FBNeo Stub NRO ==="
...
NRO_OUTPUT="$BUILD_DIR/GBAStationFBNeoStub.nro"
ELF2NRO="$DEVKITPRO/tools/bin/elf2nro"
NACPTOOL="$DEVKITPRO/tools/bin/nacptool"

# Create NACP
NACP_FILE="$BUILD_DIR/GBAStationFBNeoStub.nacp"
$NACPTOOL --create "GBAStation FBNeo Stub" "GBAStation" "1.0.3" "$NACP_FILE"
...
if [ -f "$NRO_OUTPUT" ]; then
    cp "$NRO_OUTPUT" "$ROOT_DIR/GBAStationFBNeoStub.nro"
    echo "======================================"
    echo "Build successful!"
    echo "Output: $NRO_OUTPUT"
    echo "Copied to: $ROOT_DIR/GBAStationFBNeoStub.nro"
    echo "======================================"
else
    echo "Error: GBAStationFBNeoStub.nro not found"
    exit 1
fi
